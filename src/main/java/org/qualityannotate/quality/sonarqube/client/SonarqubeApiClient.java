...
public interface SonarqubeApiClient {
    ...
    ComponentMeasures getComponentMeasures(@QueryParam("component") String project,
            @QueryParam("pullRequest") String prId, @QueryParam("metricKeys") String metricKeys,
            @QueryParam("additionalFields") @DefaultValue("metrics") String additionalFields);
    ...
    default IssueSearch getIssuesSearch(String project, String prId, String branch) {
        return getIssuesSearch(project, prId, branch, "_all", "severities,types", 1, 500, "false", "FILE_LINE",
                // INFO got removed here
                "MINOR,MAJOR,CRITICAL,BLOCKER", "OPEN,REOPENED,CONFIRMED");
    }

    /**
     * <a href="https://sonarcloud.io/web_api/api/issues/search">docs</a> TODO onComponentOnly might be
     * interesting to retrieve global metrics
     *
     * @param project          Component key. E.g. my_project
     * @param prId             Pull request id. Not available in the community edition. E.g. 1234
     ...
     * @param facets           Comma-separated list of the facets to be computed. No facet is computed
     *                         by default.
    ...
    IssueSearch getIssuesSearch(@QueryParam("componentKeys") String project,
            @Nullable @QueryParam("pullRequest") String prId, @Nullable @QueryParam("branch") String branch,
